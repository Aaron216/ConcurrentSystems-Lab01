...
int main(void) {
    ...
    if ( my_rank == 0) {
        ...
        printf("\n");
        /* (46) Gather from each process each sum to send back to process 0 and store each processes' sum in array sum_proc */
...
int Global_sum(
    ...
    if (my_rank >= floor_log_p) {
        ...
        partner = my_rank + floor_log_p;
        if (partner < comm_sz) {
            /* (85) Recv value from partner into recvtemp */
    ...
    if (my_rank < floor_log_p) {
        while (bitmask < floor_log_p) {
            ...
            /* (94) Send value from my_sum to partner process and receive value from partner into recvtemp */
            MPI_Sendrecv(&my_sum, 1, MPI_INT, partner, 0, &recvtemp, 1, MPI_INT, partner, 0, comm, MPI_STATUS_IGNORE);
            my_sum += recvtemp;
            bitmask <<= 1;
        }  /* while */
    }
    ...
    if (my_rank >= floor_log_p) {
        partner = my_rank - floor_log_p;
        /* (102) Recv value from partner into my_sum */
        MPI_Recv(&my_sum, 1, MPI_INT, partner, 0, comm, MPI_STATUS_IGNORE);
    }
    else {
        partner = my_rank + floor_log_p;
        if (partner < comm_sz) {
            /* (106) Send value in my_sum to partner process */
            MPI_Send(&my_sum, 1, MPI_INT, partner, 0, comm);
        }
    }
